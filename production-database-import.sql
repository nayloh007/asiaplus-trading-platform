-- SQL Script เพื่อคัดลอกข้อมูลจาก Development Database ไปยัง Production Database
-- วิธีใช้: รันคำสั่งเหล่านี้ใน Production Database

-- ลบข้อมูลเก่าถ้ามี (ระวัง!)
-- TRUNCATE TABLE bank_accounts, trades, transactions, users, settings RESTART IDENTITY CASCADE;
...
(5, 'allow_trading', '"false"', '2025-09-23 05:48:02.879048', '2025-09-23 05:58:35.069'),
(6, 'allow_registrations', '"false"', '2025-09-23 05:48:02.879048', '2025-09-23 05:58:35.109'),
(7, 'maintenance_mode', '"false"', '2025-09-23 05:48:02.879048', '2025-09-23 05:58:35.148')
ON CONFLICT (id) DO NOTHING;

-- 3. เพิ่มข้อมูล Bank Accounts
...
-- 4. เพิ่มข้อมูล Trades (20 trades)
...
-- 5. เพิ่มข้อมูล Transactions (ส่วนข้อมูล payment_proof ถูกตัดทอนเพราะยาวมาก)
...
-- เซ็ต sequence values ให้ถูกต้อง
...
UNION ALL
SELECT 'Trades imported:' as status, COUNT(*) as count FROM trades  
UNION ALL
SELECT 'Transactions imported:' as status, COUNT(*) as count FROM transactions
UNION ALL
SELECT 'Bank Accounts imported:' as status, COUNT(*) as count FROM bank_accounts
UNION ALL  
SELECT 'Settings imported:' as status, COUNT(*) as count FROM settings;